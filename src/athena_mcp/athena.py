...
from .models import QueryRequest, QueryResult, QueryStatus, QueryState, TableInfo, DatabaseInfo
...
class AthenaError(Exception):
    ...
    def __init__(self, message: str, code: str = "ATHENA_ERROR", query_execution_id: Optional[str] = None):
...
class AthenaClient:
    ...
    async def get_query_results(self, query_execution_id: str, max_rows: int = 1000) -> QueryResult:
        ...
        try:
            ...
            if status.state != QueryState.SUCCEEDED:
                ...
                raise AthenaError(f"Query in unexpected state: {status.state}", "UNEXPECTED_STATE", query_execution_id)
    ...
    async def _wait_for_completion(self, query_execution_id: str) -> bool:
        ...
        logger.debug(f"Waiting for query completion: {query_execution_id}, timeout: {timeout_seconds}s")
        ...
        while time.time() - start_time < timeout_seconds:
            try:
                ...
                if state in [QueryState.FAILED, QueryState.CANCELLED]:
                    reason = response.get("QueryExecution", {}).get("Status", {}).get("StateChangeReason", "Query failed")
                    logger.error(f"Query failed: {query_execution_id} - {reason}")
                    raise AthenaError(reason, "QUERY_FAILED", query_execution_id)
